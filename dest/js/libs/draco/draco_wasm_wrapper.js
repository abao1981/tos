...
$jscomp.defineProperty = $jscomp.ASSUME_ES5 || "function" == typeof Object.defineProperties ? Object.defineProperty : function (d, f, n) { d != Array.prototype && d != Object.prototype && (d[f] = n.value); };
...
$jscomp.polyfill = function (d, f, n, w) { if (f) {
    n = $jscomp.global;
    d = d.split(".");
...
$jscomp.polyfill("Math.imul", function (d) { return d ? d : function (f, d) { f = Number(f); d = Number(d); var n = f & 65535, g = d & 65535; return n * g + ((f >>> 16 & 65535) * g + n * (d >>> 16 & 65535) << 16 >>> 0) | 0; }; }, "es6", "es3");
$jscomp.polyfill("Math.clz32", function (d) { return d ? d : function (f) { f = Number(f) >>> 0; if (0 === f)
    return 32; var d = 0; 0 === (f & 4294901760) && (f <<= 16, d += 16); 0 === (f & 4278190080) && (f <<= 8, d += 8); 0 === (f & 4026531840) && (f <<= 4, d += 4); 0 === (f & 3221225472) && (f <<= 2, d += 2); 0 === (f & 2147483648) && d++; return d; }; }, "es6", "es3");
$jscomp.polyfill("Math.trunc", function (d) { return d ? d : function (d) { d = Number(d); if (isNaN(d) || Infinity === d || -Infinity === d || 0 === d)
    return d; var f = Math.floor(Math.abs(d)); return 0 > d ? -f : f; }; }, "es6", "es3");
...
$jscomp.arrayIterator = function (d) { var f = 0; return $jscomp.iteratorPrototype(function () { return f < d.length ? { done: !1, value: d[f++] } : { done: !0 }; }); };
...
$jscomp.makeIterator = function (d) { $jscomp.initSymbolIterator(); var f = d[Symbol.iterator]; return f ? f.call(d) : $jscomp.arrayIterator(d); };
$jscomp.FORCE_POLYFILL_PROMISE = !1;
$jscomp.polyfill("Promise", function (d) {
    function f() { this.batch_ = null; }
    function n(d) { return d instanceof g ? d : new g(function (f, D) { f(d); }); }
    if (d && !$jscomp.FORCE_POLYFILL_PROMISE)
        return d;
    f.prototype.asyncExecute = function (d) { null == this.batch_ && (this.batch_ = [], this.asyncExecuteBatch_()); this.batch_.push(d); return this; };
    f.prototype.asyncExecuteBatch_ = function () { var d = this; this.asyncExecuteFunction(function () { d.executeBatch_(); }); };
    var w = $jscomp.global.setTimeout;
    f.prototype.asyncExecuteFunction = function (d) {
        w(d, 0);
    };
    f.prototype.executeBatch_ = function () { for (; this.batch_ && this.batch_.length;) {
        var d = this.batch_;
        this.batch_ = [];
    ...
    f.prototype.asyncThrow_ = function (d) { this.asyncExecuteFunction(function () { throw d; }); };
    var g = function (d) { this.state_ = 0; this.result_ = void 0; this.onSettledCallbacks_ = []; var f = this.createResolveAndReject_(); try {
    ...
    g.prototype.createResolveAndReject_ =
        function () { function d(d) { return function (D) { g || (g = !0, d.call(f, D)); }; } var f = this, g = !1; return { resolve: d(this.resolveTo_), reject: d(this.reject_) }; };
    g.prototype.resolveTo_ = function (d) { if (d === this)
    ...
    g.prototype.reject_ = function (d) { this.settle_(2, d); };
    g.prototype.fulfill_ = function (d) { this.settle_(1, d); };
    g.prototype.settle_ = function (d, f) { if (0 != this.state_)
        throw Error("Cannot settle(" + d + ", " + f | "): Promise already settled in state" + this.state_); this.state_ = d; this.result_ = f; this.executeOnSettledCallbacks_(); };
    ...
    var ma = new f;
    g.prototype.settleSameAsPromise_ = function (d) { var f = this.createResolveAndReject_(); d.callWhenSettled_(f.resolve, f.reject); };
    g.prototype.settleSameAsThenable_ = function (d, f) { var g = this.createResolveAndReject_(); try {
    ...
    g.prototype.then = function (d, f) {
        function u(d, f) { return "function" == typeof d ? function (f) { try {
            n(d(f));
        }
        catch (ea) {
            D(ea);
        } } : f; }
        var n, D, w = new g(function (d, f) { n = d; D = f; });
        this.callWhenSettled_(u(d, n), u(f, D));
        return w;
    };
    g.prototype.catch = function (d) { return this.then(void 0, d); };
    g.prototype.callWhenSettled_ = function (d, f) { function g() { switch (n.state_) {
        case 1:
            d(n.result_);
            break;
        case 2:
            f(n.result_);
            break;
        default: throw Error("Unexpected state: " + n.state_);
    } } var n = this; null == this.onSettledCallbacks_ ? ma.asyncExecute(g) : this.onSettledCallbacks_.push(function () { ma.asyncExecute(g); }); };
    g.resolve = n;
    g.reject = function (d) { return new g(function (f, g) { g(d); }); };
    g.race = function (d) { return new g(function (f, g) { for (var u = $jscomp.makeIterator(d), w = u.next(); !w.done; w = u.next())
        n(w.value).callWhenSettled_(f, g); }); };
    g.all = function (d) { var f = $jscomp.makeIterator(d), u = f.next(); return u.done ? n([]) : new g(function (d, g) { function w(f) { return function (g) { D[f] = g; Q--; 0 == Q && d(D); }; } var D = [], Q = 0; do
        D.push(void 0), Q++, n(u.value).callWhenSettled_(w(D.length - 1), g), u = f.next();
    while (!u.done); }); };
    return g;
}, "es6", "es3");
var DracoDecoderModule = function (d) {
    function f(a, b) { a || W("Assertion failed: " + b); }
    function n(e, b) { if (0 === b || !e)
        return ""; for (var c = 0, l, d = 0;;) {
        l = T[e + d >> 0];
        c |= l;
        if (0 == l && !b)
            break;
    ...
    function w(a) { return a.replace(/__Z[\w\d_]+/g, function (a) { return a === a ? a : a + " [" + a + "]"; }); }
    function g() {
        a: {
            var e = Error();
            if (!e.stack) {
    ...
    function ma(a, b) { 0 < a % b && (a += b - a % b); return a; }
    function D() { a.HEAP8 = fa = new Int8Array(F); a.HEAP16 = za = new Int16Array(F); a.HEAP32 = x = new Int32Array(F); a.HEAPU8 = T = new Uint8Array(F); a.HEAPU16 = Oa = new Uint16Array(F); a.HEAPU32 = Pa = new Uint32Array(F); a.HEAPF32 = Qa = new Float32Array(F); a.HEAPF64 = Ra = new Float64Array(F); }
    function Ma() {
    ...
    function la(e) { ha++; a.monitorRunDependencies && a.monitorRunDependencies(ha); }
    function Na(e) {
        ha--;
        a.monitorRunDependencies && a.monitorRunDependencies(ha);
        0 == ha && (null !== Ba && (clearInterval(Ba), Ba = null), sa && (e = sa, sa = null, e()));
    }
    function na() { return !!na.uncaught_exception; }
    function qa() {
        var e = A.last;
        if (!e)
    ...
    function ra(e, b) { ra.seen || (ra.seen = {}); e in ra.seen || (a.dynCall_v(b), ra.seen[e] = 1); }
    function ea(a) { this.name = "ExitStatus"; this.message = "Program terminated with exit(" + a + ")"; this.status = a; }
    function Da(e) {
        function b() {
        ...
        if (!(0 < ha)) {
            ...
            u($a);
            0 < ha || a.calledRun || (a.setStatus ? (a.setStatus("Running..."), setTimeout(function () { setTimeout(function () { a.setStatus(""); }, 1); b(); }, 1)) : b());
        }
    }
    function W(e) {
        if (a.onAbort)
        ...
        ab && ab.forEach(function (a) { b = a(b, e); });
        throw b;
    }
    function p() { }
    function t(a) { return (a || p).__cache__; }
    function X(a, b) { var c = t(b), e = c[a]; if (e)
        return e; e = Object.create((b || p).prototype); e.ptr = a; return c[a] = e; }
    function Y(a) {
        if ("string" === typeof a) {
    ...
    function G() { this.ptr = lb(); t(G)[this.ptr] = this; }
    function H() { this.ptr = mb(); t(H)[this.ptr] = this; }
    function I() { this.ptr = nb(); t(I)[this.ptr] = this; }
    function J() { this.ptr = ob(); t(J)[this.ptr] = this; }
    function K() { this.ptr = pb(); t(K)[this.ptr] = this; }
    function q() { this.ptr = qb(); t(q)[this.ptr] = this; }
    function P() { this.ptr = rb(); t(P)[this.ptr] = this; }
    function z() { this.ptr = sb(); t(z)[this.ptr] = this; }
    function L() { this.ptr = tb(); t(L)[this.ptr] = this; }
    function r() {
        this.ptr =
            ub();
        t(r)[this.ptr] = this;
    }
    function M() { this.ptr = vb(); t(M)[this.ptr] = this; }
    function N() { this.ptr = wb(); t(N)[this.ptr] = this; }
    function Z() { this.ptr = xb(); t(Z)[this.ptr] = this; }
    function R() { this.ptr = yb(); t(R)[this.ptr] = this; }
    function h() { this.ptr = zb(); t(h)[this.ptr] = this; }
    function C() { this.ptr = Ab(); t(C)[this.ptr] = this; }
    function ca() { throw "cannot construct a VoidPtr, no constructor in IDL"; }
    function O() { this.ptr = Bb(); t(O)[this.ptr] = this; }
    function S() { this.ptr = Cb(); t(S)[this.ptr] = this; }
    var a = d = d || {}, bb = !1, cb = !1;
    a.onRuntimeInitialized =
        function () { bb = !0; if (cb && "function" === typeof a.onModuleLoaded)
            a.onModuleLoaded(a); };
    a.onModuleParsed = function () { cb = !0; if (bb && "function" === typeof a.onModuleLoaded)
        a.onModuleLoaded(a); };
    ...
    if (pa) {
        ...
        var Ea, Fa;
        a.read = function (a, b) { Ea || (Ea = require("fs")); Fa || (Fa = require("path")); a = Fa.normalize(a); a = Ea.readFileSync(a); return b ? a : a.toString(); };
        a.readBinary = function (e) { e = a.read(e, !0); e.buffer || (e = new Uint8Array(e)); f(e.buffer); return e; };
        ...
        process.on("uncaughtException", function (a) { if (!(a instanceof ea))
            throw a; });
        a.inspect = function () { return "[Emscripten Module object]"; };
    }
    else if (wa)
        a.print ||
            (a.print = print), "undefined" != typeof printErr && (a.printErr = printErr), a.read = "undefined" != typeof read ? function (a) { return read(a); } : function () { throw "no read() available"; }, a.readBinary = function (a) { if ("function" === typeof readbuffer)
            return new Uint8Array(readbuffer(a)); a = read(a, "binary"); f("object" === typeof a); return a; }, "undefined" != typeof scriptArgs ? a.arguments = scriptArgs : "undefined" != typeof arguments && (a.arguments = arguments), "function" === typeof quit && (a.quit = function (a, b) { quit(a); });
    else if (oa || ka)
        a.read =
            ...
            "undefined" !== typeof console ? (a.print || (a.print = function (a) { console.log(a); }), a.printErr || (a.printErr = function (a) { console.warn(a); })) : a.print || (a.print = function (a) { }), "undefined" === typeof a.setWindowTitle && (a.setWindowTitle = function (a) { document.title = a; });
    else
    ...
    a.quit || (a.quit = function (a, b) { throw b; });
    a.print =
        a.print;
    ...
    va = void 0;
    var m = { setTempRet0: function (a) { return tempRet0 = a; }, getTempRet0: function () { return tempRet0; }, stackSave: function () { return U; }, stackRestore: function (a) { U = a; }, getNativeTypeSize: function (a) {
            switch (a) {
                case "i1":
                case "i8": return 1;
                case "i16": return 2;
                ...
                default: return "*" === a[a.length - 1] ? m.QUANTUM_SIZE : "i" === a[0] ?
                    (a = parseInt(a.substr(1)), f(0 === a % 8), a / 8) : 0;
            }
        }, getNativeFieldSize: function (a) { return Math.max(m.getNativeTypeSize(a), m.QUANTUM_SIZE); }, STACK_ALIGN: 16, prepVararg: function (a, b) { "double" === b || "i64" === b ? a & 7 && (f(4 === (a & 7)), a += 4) : f(0 === (a & 3)); return a; }, getAlignSize: function (a, b, c) { return c || "i64" != a && "double" != a ? a ? Math.min(b || (a ? m.getNativeFieldSize(a) : 0), m.QUANTUM_SIZE) : Math.min(b, 8) : 8; }, dynCall: function (e, b, c) { return c && c.length ? a["dynCall_" + e].apply(null, [b].concat(c)) : a["dynCall_" + e].call(null, b); }, functionPointers: [],
        addFunction: function (a) { for (var b = 0; b < m.functionPointers.length; b++)
            if (!m.functionPointers[b])
                return m.functionPointers[b] = a, 2 * (1 + b); throw "Finished up all reserved function pointers. Use a higher value for RESERVED_FUNCTION_POINTERS."; }, removeFunction: function (a) { m.functionPointers[(a - 2) / 2] = null; }, warnOnce: function (e) { m.warnOnce.shown || (m.warnOnce.shown = {}); m.warnOnce.shown[e] || (m.warnOnce.shown[e] = 1, a.printErr(e)); }, funcWrappers: {}, getFuncWrapper: function (a, b) {
            if (a) {
                f(b);
                m.funcWrappers[b] || (m.funcWrappers[b] =
                    {});
                var c = m.funcWrappers[b];
                c[a] || (c[a] = 1 === b.length ? function () { return m.dynCall(b, a); } : 2 === b.length ? function (c) { return m.dynCall(b, a, [c]); } : function () { return m.dynCall(b, a, Array.prototype.slice.call(arguments)); });
                return c[a];
            }
        }, getCompilerSetting: function (a) { throw "You must build with -s RETAIN_COMPILER_SETTINGS=1 for Runtime.getCompilerSetting or emscripten_get_compiler_setting to work"; }, stackAlloc: function (a) { var b = U; U = U + a | 0; U = U + 15 & -16; return b; }, staticAlloc: function (a) {
            var b = aa;
            aa = aa + a | 0;
            aa = aa + 15 &
                -16;
    ...
    a.reallocBuffer || (a.reallocBuffer = function (a) { try {
        if (ArrayBuffer.transfer)
            var b = ArrayBuffer.transfer(F, a);
        else {
            var c = fa;
            b = new ArrayBuffer(a);
    ...
    try {
        ...
        Ja = function (a) { return a.byteLength; };
    }
    ...
    if (a.buffer)
        ...
        "object" === typeof WebAssembly && "function" === typeof WebAssembly.Memory ? (a.wasmMemory = new WebAssembly.Memory({ initial: y / Aa }), F = a.wasmMemory.buffer) : F = new ArrayBuffer(y);
    D();
    x[0] = 1668509029;
    za[1] = 25459;
    if (115 !== T[2] || 99 !== T[3])
    ...
    (function () {
        ...
        function b() { return a.wasmBinary || !oa && !ka || "function" !== typeof fetch ? new Promise(function (a, b) { a(e()); }) : fetch(d, { credentials: "same-origin" }).then(function (a) { if (!a.ok)
            throw "failed to load wasm binary file at '" + d + "'"; return a.arrayBuffer(); }).catch(function () { return e(); }); }
        function c(c, e, l) {
            function f(b, c) {
                h = b.exports;
                if (h.memory) {
            ...
            function E(a) { f(a.instance, a.module); }
            function k(c) { b().then(function (a) { return WebAssembly.instantiate(a, g); }).then(c).catch(function (b) { a.printErr("failed to asynchronously prepare wasm: " + b); W(b); }); }
            ...
            a.wasmBinary || "function" !== typeof WebAssembly.instantiateStreaming || 0 === d.indexOf("data:") || "function" !== typeof fetch ? k(E) : WebAssembly.instantiateStreaming(fetch(d, { credentials: "same-origin" }), g).then(E).catch(function (b) { a.printErr("wasm streaming compile failed: " + b); a.printErr("falling back to ArrayBuffer instantiation"); k(E); });
            return {};
        }
        ...
        var g = { global: null, env: null, asm2wasm: { "f64-rem": function (a, b) { return a % b; }, "debugger": function () { debugger; } }, parent: a }, h = null;
        a.asmPreload = a.asm;
        ...
        a.asm = function (b, e, d) {
            if (!e.table) {
                ...
                e.table = "object" === typeof WebAssembly && "function" === typeof WebAssembly.Table ? void 0 !== f ? new WebAssembly.Table({ initial: l, maximum: f, element: "anyfunc" }) : new WebAssembly.Table({ initial: l,
                    element: "anyfunc" }) : Array(l);
                a.wasmTable = e.table;
            }
    ...
    var Eb = aa;
    aa += 16;
    var A = { last: 0, caught: [], infos: {}, deAdjust: function (a) {
            if (!a || A.infos[a])
                return a;
            for (var b in A.infos)
                if (A.infos[b].adjusted === a)
                    return b;
            return a;
        }, addRef: function (a) { a && A.infos[a].refcount++; }, decRef: function (e) { if (e) {
            var b = A.infos[e];
            f(0 < b.refcount);
            b.refcount--;
            0 !== b.refcount || b.rethrown || (b.destructor && a.dynCall_vi(b.destructor, e), delete A.infos[e], ___cxa_free_exception(e));
        } }, clearRef: function (a) { a && (A.infos[a].refcount = 0); } }, v = { varargs: 0, get: function (a) { v.varargs += 4; return x[v.varargs - 4 >> 2]; }, getStr: function () { return n(v.get()); }, get64: function () {
            var a = v.get(), b = v.get();
            0 <= a ?
                f(0 === b) : f(-1 === b);
            return a;
        }, getZero: function () { f(0 === v.get()); } }, ya = {}, La = 1;
    db.push(function () { var e = a._fflush; e && e(0); if (e = Q.printChar) {
        var b = Q.buffers;
        b[1].length && e(1, 10);
        b[2].length && e(2, 10);
    } });
    ...
    a.asmGlobalArg = { Math: Math, Int8Array: Int8Array, Int16Array: Int16Array, Int32Array: Int32Array, Uint8Array: Uint8Array, Uint16Array: Uint16Array, Uint32Array: Uint32Array, Float32Array: Float32Array,
        Float64Array: Float64Array, NaN: NaN, Infinity: Infinity, byteLength: Ja };
    a.asmLibraryArg = { abort: W, assert: f, enlargeMemory: Ma, getTotalMemory: function () { return y; }, abortOnCannotGrowMemory: function () { W("Cannot enlarge memory arrays. Either (1) compile with  -s TOTAL_MEMORY=X  with X higher than the current value " + y + ", (2) compile with  -s ALLOW_MEMORY_GROWTH=1  which allows increasing the size at runtime, or (3) if you want malloc to return NULL (0) instead of this abort, compile with  -s ABORTING_MALLOC=0 "); },
        invoke_ii: function (e, b) { try {
            ...
            if ("number" !== typeof l && "longjmp" !== l)
                throw l;
            a.setThrew(1, 0);
        } }, invoke_iiii: function (e, b, c, d) { try {
            ...
            try {
                ...
                if ("number" !==
                    typeof ja && "longjmp" !== ja)
                    throw ja;
                a.setThrew(1, 0);
            }
        }, invoke_v: function (e) { try {
            a.dynCall_v(e);
        }
        catch (b) {
        ...
        }, invoke_viiii: function (e, b, c, d, f) { try {
            ...
            if ("number" !== typeof ta && "longjmp" !== ta)
                throw ta;
            a.setThrew(1, 0);
        } }, invoke_viiiii: function (e, b, c, d, f, g) { try {
            ...
            if ("number" !== typeof ia && "longjmp" !== ia)
                throw ia;
            a.setThrew(1, 0);
        } }, invoke_viiiiii: function (e, b, c, d, f, g, h) { try {
        ...
        }, ___gxx_personality_v0: function () { }, ___resumeException: function (a) { A.last || (A.last = a); throw a + " - Exception catching is disabled, this exception cannot be caught. Compile with -s DISABLE_EXCEPTION_CATCHING=0 or DISABLE_EXCEPTION_CATCHING=2 to catch."; }, ___setErrNo: function (e) { a.___errno_location && (x[a.___errno_location() >> 2] = e); return e; },
        ___syscall140: function (a, b) { v.varargs = b; try {
        ...
        } }, ___syscall146: Q, ___syscall54: function (a, b) { v.varargs = b; return 0; }, ___syscall6: function (a, b) {
            v.varargs = b;
            try {
                var c = v.getStreamFromFD();
                FS.close(c);
        ...
        }, _abort: function () { a.abort(); }, _emscripten_memcpy_big: function (a, b, c) { T.set(T.subarray(b, b + c), a); return a; }, _pthread_getspecific: function (a) { return ya[a] || 0; }, _pthread_key_create: function (a, b) { if (0 == a)
            return 22; x[a >> 2] = La; ya[La] = 0; La++; return 0; }, _pthread_once: ra, _pthread_setspecific: function (a, b) { if (!(a in ya))
            return 22; ya[a] = b; return 0; }, DYNAMICTOP_PTR: ba, tempDoublePtr: Eb, ABORT: ua, STACKTOP: U, STACK_MAX: xa };
    ...
    a._emscripten_get_global_libc = function () { return a.asm._emscripten_get_global_libc.apply(null, arguments); };
    ...
    if (V)
        if ("function" === typeof a.locateFile ? V = a.locateFile(V) : a.memoryInitializerPrefixURL && (V = a.memoryInitializerPrefixURL + V), pa || wa) {
            ...
            var gb = function () {
                a.readAsync(V, fb, function () { throw "could not load memory initializer " + V; });
            };
            la("memory initializer");
            var fb = function (d) { d.byteLength && (d = new Uint8Array(d)); T.set(d, m.GLOBAL_BASE); a.memoryInitializerRequest && delete a.memoryInitializerRequest.response; Na("memory initializer"); };
    ...
    a.then = function (d) { if (a.calledRun)
        d(a);
    else {
        var b = a.onRuntimeInitialized;
        a.onRuntimeInitialized = function () { b && b(); d(a); };
    } return a; };
    ea.prototype = Error();
    ea.prototype.constructor = ea;
    var Za = null;
    sa = function b() { a.calledRun || Da(); a.calledRun || (sa = b); };
    a.run = Da;
    a.exit = function (b, c) {
    ...
    a.getCache = t;
    a.wrapPointer = X;
    a.castObject = function (a, c) { return X(a.ptr, c); };
    a.NULL = X(0);
    ...
    a.compare = function (a, c) { return a.ptr === c.ptr; };
    a.getPointer = function (a) { return a.ptr; };
    a.getClass = function (a) { return a.__class__; };
    var k = { buffer: 0, size: 0, pos: 0, temps: [], needed: 0, prepare: function () { if (k.needed) {
            ...
            k.size += k.needed;
            k.needed = 0;
        } k.buffer || (k.size += 128, k.buffer = a._malloc(k.size), f(k.buffer)); k.pos = 0; }, alloc: function (b, c) {
            f(k.buffer);
            ...
            return c;
        }, copy: function (a, c, d) { switch (c.BYTES_PER_ELEMENT) {
            case 2:
                d >>= 1;
                break;
            case 4:
    ...
    B.__cache__ = {};
    a.Status = B;
    B.prototype.code = B.prototype.code = function () { return ld(this.ptr); };
    B.prototype.ok = B.prototype.ok = function () { return !!nd(this.ptr); };
    B.prototype.error_msg =
        B.prototype.error_msg = function () { return n(md(this.ptr)); };
    B.prototype.__destroy__ = B.prototype.__destroy__ = function () { kd(this.ptr); };
    G.prototype = Object.create(p.prototype);
    G.prototype.constructor = G;
    G.prototype.__class__ = G;
    G.__cache__ = {};
    a.DracoUInt16Array = G;
    G.prototype.GetValue = G.prototype.GetValue = function (a) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); return zc(b, a); };
    G.prototype.size = G.prototype.size = function () { return Bc(this.ptr); };
    G.prototype.__destroy__ = G.prototype.__destroy__ = function () { Ac(this.ptr); };
    H.prototype = Object.create(p.prototype);
    H.prototype.constructor = H;
    H.prototype.__class__ = H;
    H.__cache__ = {};
    a.PointCloud = H;
    H.prototype.num_attributes = H.prototype.num_attributes = function () { return id(this.ptr); };
    H.prototype.num_points = H.prototype.num_points = function () { return jd(this.ptr); };
    H.prototype.__destroy__ = H.prototype.__destroy__ = function () { hd(this.ptr); };
    ...
    I.prototype.GetValue =
        I.prototype.GetValue = function (a) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); return Fc(b, a); };
    I.prototype.size = I.prototype.size = function () { return Hc(this.ptr); };
    I.prototype.__destroy__ = I.prototype.__destroy__ = function () { Gc(this.ptr); };
    J.prototype = Object.create(p.prototype);
    J.prototype.constructor = J;
    J.prototype.__class__ = J;
    J.__cache__ = {};
    a.DracoUInt32Array = J;
    J.prototype.GetValue = J.prototype.GetValue = function (a) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); return Cc(b, a); };
    J.prototype.size = J.prototype.size =
        function () { return Ec(this.ptr); };
    J.prototype.__destroy__ = J.prototype.__destroy__ = function () { Dc(this.ptr); };
    K.prototype = Object.create(p.prototype);
    K.prototype.constructor = K;
    K.prototype.__class__ = K;
    K.__cache__ = {};
    a.AttributeOctahedronTransform = K;
    K.prototype.InitFromAttribute = K.prototype.InitFromAttribute = function (a) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); return !!Fb(b, a); };
    K.prototype.quantization_bits = K.prototype.quantization_bits = function () { return Hb(this.ptr); };
    K.prototype.__destroy__ = K.prototype.__destroy__ =
        function () { Gb(this.ptr); };
    q.prototype = Object.create(p.prototype);
    q.prototype.constructor = q;
    q.prototype.__class__ = q;
    q.__cache__ = {};
    a.PointAttribute = q;
    q.prototype.size = q.prototype.size = function () { return fd(this.ptr); };
    q.prototype.GetAttributeTransformData = q.prototype.GetAttributeTransformData = function () { return X(Yc(this.ptr), P); };
    q.prototype.attribute_type = q.prototype.attribute_type = function () { return $c(this.ptr); };
    q.prototype.data_type = q.prototype.data_type = function () { return cd(this.ptr); };
    q.prototype.num_components =
        q.prototype.num_components = function () { return ed(this.ptr); };
    q.prototype.normalized = q.prototype.normalized = function () { return !!dd(this.ptr); };
    q.prototype.byte_stride = q.prototype.byte_stride = function () { return bd(this.ptr); };
    q.prototype.byte_offset = q.prototype.byte_offset = function () { return ad(this.ptr); };
    q.prototype.unique_id = q.prototype.unique_id = function () { return gd(this.ptr); };
    q.prototype.__destroy__ = q.prototype.__destroy__ = function () { Zc(this.ptr); };
    P.prototype = Object.create(p.prototype);
    ...
    P.prototype.transform_type = P.prototype.transform_type = function () { return Ob(this.ptr); };
    P.prototype.__destroy__ = P.prototype.__destroy__ = function () { Nb(this.ptr); };
    z.prototype = Object.create(p.prototype);
    z.prototype.constructor = z;
    z.prototype.__class__ = z;
    z.__cache__ = {};
    a.AttributeQuantizationTransform = z;
    z.prototype.InitFromAttribute = z.prototype.InitFromAttribute = function (a) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); return !!Ib(b, a); };
    z.prototype.quantization_bits = z.prototype.quantization_bits = function () { return Lb(this.ptr); };
    z.prototype.min_value = z.prototype.min_value = function (a) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); return Kb(b, a); };
    z.prototype.range = z.prototype.range = function () { return Mb(this.ptr); };
    z.prototype.__destroy__ = z.prototype.__destroy__ = function () { Jb(this.ptr); };
    ...
    L.prototype.GetValue = L.prototype.GetValue =
        function (a) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); return wc(b, a); };
    L.prototype.size = L.prototype.size = function () { return yc(this.ptr); };
    L.prototype.__destroy__ = L.prototype.__destroy__ = function () { xc(this.ptr); };
    ...
    r.prototype.HasIntEntry = r.prototype.HasIntEntry = function (a, c) { var b = this.ptr; k.prepare(); a && "object" === typeof a && (a = a.ptr); c = c && "object" === typeof c ? c.ptr : Y(c); return !!Tc(b, a, c); };
    r.prototype.GetIntEntry = r.prototype.GetIntEntry = function (a, c) { var b = this.ptr; k.prepare(); a && "object" === typeof a && (a = a.ptr); c = c && "object" === typeof c ? c.ptr : Y(c); return Pc(b, a, c); };
    ...
    r.prototype.GetDoubleEntry = r.prototype.GetDoubleEntry = function (a, c) { var b = this.ptr; k.prepare(); a && "object" === typeof a && (a = a.ptr); c = c && "object" === typeof c ? c.ptr : Y(c); return Nc(b, a, c); };
    r.prototype.HasStringEntry = r.prototype.HasStringEntry = function (a, c) { var b = this.ptr; k.prepare(); a && "object" === typeof a && (a = a.ptr); c = c && "object" === typeof c ? c.ptr : Y(c); return !!Uc(b, a, c); };
    ...
    r.prototype.NumEntries = r.prototype.NumEntries = function (a) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); return Vc(b, a); };
    r.prototype.GetEntryName = r.prototype.GetEntryName = function (a, c) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); return n(Oc(b, a, c)); };
    r.prototype.__destroy__ = r.prototype.__destroy__ = function () { Wc(this.ptr); };
    ...
    M.__cache__ = {};
    a.DracoInt16Array = M;
    M.prototype.GetValue = M.prototype.GetValue = function (a) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); return qc(b, a); };
    M.prototype.size = M.prototype.size = function () { return sc(this.ptr); };
    M.prototype.__destroy__ = M.prototype.__destroy__ = function () { rc(this.ptr); };
    ...
    N.prototype.size = N.prototype.size = function () { return pc(this.ptr); };
    N.prototype.__destroy__ = N.prototype.__destroy__ = function () { oc(this.ptr); };
    Z.prototype = Object.create(p.prototype);
    Z.prototype.constructor = Z;
    Z.prototype.__class__ = Z;
    Z.__cache__ = {};
    a.GeometryAttribute = Z;
    Z.prototype.__destroy__ = Z.prototype.__destroy__ = function () { Ic(this.ptr); };
    R.prototype = Object.create(p.prototype);
    R.prototype.constructor = R;
    R.prototype.__class__ = R;
    R.__cache__ = {};
    a.DecoderBuffer = R;
    R.prototype.Init = R.prototype.Init = function (a, c) { var b = this.ptr; k.prepare(); if ("object" == typeof a && "object" === typeof a) {
        var d = k.alloc(a, fa);
        k.copy(a, fa, d);
        a = d;
    } c && "object" === typeof c && (c = c.ptr); Pb(b, a, c); };
    R.prototype.__destroy__ = R.prototype.__destroy__ = function () { Qb(this.ptr); };
    h.prototype = Object.create(p.prototype);
    h.prototype.constructor = h;
    h.prototype.__class__ = h;
    h.__cache__ = {};
    ...
    h.prototype.DecodeBufferToPointCloud = h.prototype.DecodeBufferToPointCloud = function (a, c) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); return X(Sb(b, a, c), B); };
    h.prototype.DecodeBufferToMesh = h.prototype.DecodeBufferToMesh = function (a, c) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); return X(Rb(b, a, c), B); };
    ...
    h.prototype.GetAttributeIdByName = h.prototype.GetAttributeIdByName = function (a, c) { var b = this.ptr; k.prepare(); a && "object" === typeof a && (a = a.ptr); c = c && "object" === typeof c ? c.ptr : Y(c); return Xb(b, a, c); };
    ...
    h.prototype.GetAttribute = h.prototype.GetAttribute = function (a, c) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); return X(gc(b, a, c), q); };
    h.prototype.GetAttributeByUniqueId = h.prototype.GetAttributeByUniqueId = function (a, c) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); return X(Tb(b, a, c), q); };
    h.prototype.GetMetadata = h.prototype.GetMetadata = function (a) {
        var b = this.ptr;
        a && "object" === typeof a && (a = a.ptr);
        return X(jc(b, a), S);
    };
    h.prototype.GetAttributeMetadata = h.prototype.GetAttributeMetadata = function (a, c) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); return X(cc(b, a, c), S); };
    h.prototype.GetFaceFromMesh = h.prototype.GetFaceFromMesh = function (a, c, d) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); d && "object" === typeof d && (d = d.ptr); return !!ic(b, a, c, d); };
    ...
    h.prototype.GetAttributeFloat = h.prototype.GetAttributeFloat = function (a, c, d) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); d && "object" === typeof d && (d = d.ptr); return !!Vb(b, a, c, d); };
    ...
    h.prototype.GetAttributeIntForAllPoints = h.prototype.GetAttributeIntForAllPoints = function (a, c, d) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); d && "object" === typeof d && (d = d.ptr); return !!bc(b, a, c, d); };
    h.prototype.GetAttributeInt8ForAllPoints = h.prototype.GetAttributeInt8ForAllPoints = function (a, c, d) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); d && "object" === typeof d && (d = d.ptr); return !!ac(b, a, c, d); };
    h.prototype.GetAttributeUInt8ForAllPoints = h.prototype.GetAttributeUInt8ForAllPoints = function (a, c, d) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); d && "object" === typeof d && (d = d.ptr); return !!fc(b, a, c, d); };
    h.prototype.GetAttributeInt16ForAllPoints = h.prototype.GetAttributeInt16ForAllPoints = function (a, c, d) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); d && "object" === typeof d && (d = d.ptr); return !!Zb(b, a, c, d); };
    h.prototype.GetAttributeUInt16ForAllPoints =
        h.prototype.GetAttributeUInt16ForAllPoints = function (a, c, d) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); d && "object" === typeof d && (d = d.ptr); return !!dc(b, a, c, d); };
    h.prototype.GetAttributeInt32ForAllPoints = h.prototype.GetAttributeInt32ForAllPoints = function (a, c, d) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); d && "object" === typeof d && (d = d.ptr); return !!$b(b, a, c, d); };
    h.prototype.GetAttributeUInt32ForAllPoints = h.prototype.GetAttributeUInt32ForAllPoints =
        function (a, c, d) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); c && "object" === typeof c && (c = c.ptr); d && "object" === typeof d && (d = d.ptr); return !!ec(b, a, c, d); };
    h.prototype.SkipAttributeTransform = h.prototype.SkipAttributeTransform = function (a) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); lc(b, a); };
    h.prototype.__destroy__ = h.prototype.__destroy__ = function () { mc(this.ptr); };
    ...
    C.prototype.num_faces =
        C.prototype.num_faces = function () { return Lc(this.ptr); };
    C.prototype.num_attributes = C.prototype.num_attributes = function () { return Kc(this.ptr); };
    C.prototype.num_points = C.prototype.num_points = function () { return Mc(this.ptr); };
    C.prototype.__destroy__ = C.prototype.__destroy__ = function () { Jc(this.ptr); };
    ca.prototype = Object.create(p.prototype);
    ca.prototype.constructor = ca;
    ca.prototype.__class__ = ca;
    ca.__cache__ = {};
    a.VoidPtr = ca;
    ca.prototype.__destroy__ = ca.prototype.__destroy__ = function () { od(this.ptr); };
    O.prototype =
    ...
    O.__cache__ = {};
    a.DracoInt32Array = O;
    O.prototype.GetValue = O.prototype.GetValue = function (a) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); return tc(b, a); };
    O.prototype.size = O.prototype.size = function () { return vc(this.ptr); };
    O.prototype.__destroy__ = O.prototype.__destroy__ = function () { uc(this.ptr); };
    ...
    S.prototype.__destroy__ =
        S.prototype.__destroy__ = function () { Xc(this.ptr); };
    (function () {
