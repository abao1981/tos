...
var CameraControls = (function (_super) {
    ...
    function CameraControls(camera, domElement) {
        ...
        _this.viewPointMap.set('init', {
            target: _this._target.clone(),
            position:  _this._camera.position.clone(),
            zoom: _this._zoom
        })
    ...
    CameraControls.prototype.rightward = function (distance, enableTransition) {
        if (enableTransition === void 0) { enableTransition = false; }
        _v3A.setFromMatrixColumn(this._camera.matrix, 0);
        _v3A.multiplyScalar(distance);
        this._encloseToBoundary(this._targetEnd, _v3A, this.boundaryFriction);
    ...
    CameraControls.prototype.upward = function (distance, enableTransition) {
        if (enableTransition === void 0) { enableTransition = false; }
        _v3A.copy(this._camera.up);
        _v3A.multiplyScalar(distance);
        this._encloseToBoundary(this._targetEnd, _v3A, this.boundaryFriction);
        if (!enableTransition) {
            this._target.copy(this._targetEnd);
    ...
    CameraControls.prototype.reset = function (label, enableTransition) {
        if (!label) {
            console.error("CameraControls.reset's label is needed");
            return;
        }
        if (!this.viewPointMap.has(label)){
            console.error(`CameraControls.reset's label ${label} is not stored`);
            return;
        }
        if (enableTransition === void 0) { enableTransition = false; }
        let {target: {x: tx, y: ty, z:tz}, position: {x:px, y:py, z:pz}, zoom} = this.viewPointMap.get(label);
        this.setLookAt(px, py, pz, tx, ty, tz, enableTransition);
        this.zoomTo(zoom, enableTransition);
    };
    CameraControls.prototype.saveState = function (label) {
        this.viewPointMap.set(label, {
    ...
    CameraControls.prototype.toJSON = function () {
        return JSON.stringify({
            ...
            // zoom0: this._zoom0,
            viewPointMap: this.viewPointMap
        });
    };
    CameraControls.prototype.fromJSON = function (json, enableTransition) {
        ...
        this.viewPointMap = obj.viewPointMap;
