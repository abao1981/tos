...
$jscomp.polyfill = function (d, k, f, u) { if (k) {
    f = $jscomp.global;
    d = d.split(".");
...
$jscomp.polyfill("Math.imul", function (d) { return d ? d : function (d, f) { d = Number(d); f = Number(f); var k = d & 65535, h = f & 65535; return k * h + ((d >>> 16 & 65535) * h + k * (f >>> 16 & 65535) << 16 >>> 0) | 0; }; }, "es6", "es3");
$jscomp.polyfill("Math.clz32", function (d) { return d ? d : function (d) { d = Number(d) >>> 0; if (0 === d)
    return 32; var f = 0; 0 === (d & 4294901760) && (d <<= 16, f += 16); 0 === (d & 4278190080) && (d <<= 8, f += 8); 0 === (d & 4026531840) && (d <<= 4, f += 4); 0 === (d & 3221225472) && (d <<= 2, f += 2); 0 === (d & 2147483648) && f++; return f; }; }, "es6", "es3");
...
$jscomp.polyfill("Promise", function (d) {
    ...
    if (d && !$jscomp.FORCE_POLYFILL_PROMISE)
        return d;
    k.prototype.asyncExecute = function (d) { null == this.batch_ && (this.batch_ = [], this.asyncExecuteBatch_()); this.batch_.push(d); return this; };
    k.prototype.asyncExecuteBatch_ = function () { var d = this; this.asyncExecuteFunction(function () { d.executeBatch_(); }); };
    var u = $jscomp.global.setTimeout;
    k.prototype.asyncExecuteFunction = function (d) {
        u(d, 0);
    };
    k.prototype.executeBatch_ = function () { for (; this.batch_ && this.batch_.length;) {
        var d = this.batch_;
        this.batch_ = [];
    ...
    var h = function (d) { this.state_ = 0; this.result_ = void 0; this.onSettledCallbacks_ = []; var r = this.createResolveAndReject_(); try {
    ...
    h.prototype.createResolveAndReject_ =
        function () { function d(d) { return function (r) { h || (h = !0, d.call(f, r)); }; } var f = this, h = !1; return { resolve: d(this.resolveTo_), reject: d(this.reject_) }; };
    h.prototype.resolveTo_ = function (d) { if (d === this)
    ...
    h.prototype.settle_ = function (d, f) { if (0 != this.state_)
    ...
    var ha = new k;
    h.prototype.settleSameAsPromise_ = function (d) { var f = this.createResolveAndReject_(); d.callWhenSettled_(f.resolve, f.reject); };
    h.prototype.settleSameAsThenable_ = function (d, f) { var h = this.createResolveAndReject_(); try {
    ...
    h.prototype.then = function (d, f) {
        function k(d, f) { return "function" == typeof d ? function (f) { try {
            v(d(f));
        }
        catch (O) {
            r(O);
        } } : f; }
        var v, r, B = new h(function (d, f) { v = d; r = f; });
        this.callWhenSettled_(k(d, v), k(f, r));
        return B;
    };
    h.prototype.catch = function (d) { return this.then(void 0, d); };
    h.prototype.callWhenSettled_ = function (d, f) { function h() { switch (k.state_) {
        case 1:
            d(k.result_);
            break;
        case 2:
            f(k.result_);
            break;
        default: throw Error("Unexpected state: " + k.state_);
    } } var k = this; null == this.onSettledCallbacks_ ? ha.asyncExecute(h) : this.onSettledCallbacks_.push(function () { ha.asyncExecute(h); }); };
    h.resolve = f;
    h.reject = function (d) { return new h(function (f, h) { h(d); }); };
    h.race = function (d) {
        return new h(function (h, k) { for (var v = $jscomp.makeIterator(d), r = v.next(); !r.done; r = v.next())
            f(r.value).callWhenSettled_(h, k); });
    };
    h.all = function (d) { var k = $jscomp.makeIterator(d), r = k.next(); return r.done ? f([]) : new h(function (d, h) { function v(f) { return function (h) { u[f] = h; B--; 0 == B && d(u); }; } var u = [], B = 0; do
...
var DracoDecoderModule = function (d) {
    ...
    function u(a, c) { if (0 === c || !a)
        return ""; for (var b = 0, e, d = 0;;) {
        e = W[a + d >> 0];
        b |= e;
        if (0 == e && !c)
            break;
    ...
    function h(a, c) {
        for (var b = c; a[b];)
            ++b;
        if (16 < b - c && a.subarray && Ia)
            return Ia.decode(a.subarray(c, b));
        for (b =
            "";;) {
            var e = a[c++];
            if (!e)
            ...
            if (e & 128) {
                var d = a[c++] & 63;
                if (192 == (e & 224))
                    b += String.fromCharCode((e & 31) << 6 | d);
                else {
                    var f = a[c++] & 63;
                    if (224 == (e & 240))
                        e = (e & 15) << 12 | d << 6 | f;
    ...
    function r() { a.HEAP8 = ia = new Int8Array(D); a.HEAP16 = Ja = new Int16Array(D); a.HEAP32 = E = new Int32Array(D); a.HEAPU8 = W = new Uint8Array(D); a.HEAPU16 = new Uint16Array(D); a.HEAPU32 = new Uint32Array(D); a.HEAPF32 = new Float32Array(D); a.HEAPF64 = new Float64Array(D); }
    function B(e) { for (; 0 < e.length;) {
        var c = e.shift();
    ...
    function Z(e, c) { w.varargs = c; try {
        var b = w.get(), p = w.get(), d = w.get();
        e = 0;
        Z.buffers || (Z.buffers = [null, [], []], Z.printChar = function (c, b) { var e = Z.buffers[c]; f(e); 0 === b || 10 === b ? ((1 === c ? a.print : a.printErr)(h(e, 0)), e.length = 0) : e.push(b); });
        for (c = 0; c < d; c++) {
    ...
    function na(a) { this.name = "ExitStatus"; this.message = "Program terminated with exit(" + a + ")"; this.status = a; }
    function wa(e) {
        function c() { if (!a.calledRun && (a.calledRun = !0, !oa)) {
            La || (La = !0, B(Ma));
            B(Na);
        ...
        if (!(0 < ea)) {
            ...
            B(Pa);
            0 < ea || a.calledRun || (a.setStatus ? (a.setStatus("Running..."), setTimeout(function () { setTimeout(function () { a.setStatus(""); }, 1); c(); }, 1)) : c());
        }
    }
    function O(e) { if (a.onAbort)
        a.onAbort(e); void 0 !== e ? (a.print(e), a.printErr(e), e = JSON.stringify(e)) : e = ""; oa = !0; throw "abort(" + e + "). Build with -s ASSERTIONS=1 for more info."; }
    function m() { }
    function t(a) { return (a || m).__cache__; }
    function T(a, c) { var b = t(c), e = b[a]; if (e)
        return e; e = Object.create((c || m).prototype); e.ptr = a; return b[a] = e; }
    function U(a) {
        if ("string" === typeof a) {
    ...
    function F() { this.ptr = Wa(); t(F)[this.ptr] = this; }
    function G() { this.ptr = Xa(); t(G)[this.ptr] = this; }
    function H() { this.ptr = Ya(); t(H)[this.ptr] = this; }
    function I() {
        this.ptr = Za();
        t(I)[this.ptr] = this;
    }
    function J() { this.ptr = $a(); t(J)[this.ptr] = this; }
    function n() { this.ptr = ab(); t(n)[this.ptr] = this; }
    function P() { this.ptr = bb(); t(P)[this.ptr] = this; }
    function x() { this.ptr = cb(); t(x)[this.ptr] = this; }
    function K() { this.ptr = db(); t(K)[this.ptr] = this; }
    function q() { this.ptr = eb(); t(q)[this.ptr] = this; }
    function L() { this.ptr = fb(); t(L)[this.ptr] = this; }
    function M() { this.ptr = gb(); t(M)[this.ptr] = this; }
    function V() { this.ptr = hb(); t(V)[this.ptr] = this; }
    function Q() { this.ptr = ib(); t(Q)[this.ptr] = this; }
    function g() {
        this.ptr =
            jb();
        t(g)[this.ptr] = this;
    }
    function C() { this.ptr = kb(); t(C)[this.ptr] = this; }
    function X() { throw "cannot construct a VoidPtr, no constructor in IDL"; }
    function N() { this.ptr = lb(); t(N)[this.ptr] = this; }
    function R() { this.ptr = mb(); t(R)[this.ptr] = this; }
    d = d || {};
    var a = "undefined" !== typeof d ? d : {}, Qa = !1, Ra = !1;
    a.onRuntimeInitialized = function () { Qa = !0; if (Ra && "function" === typeof a.onModuleLoaded)
        a.onModuleLoaded(a); };
    a.onModuleParsed = function () { Ra = !0; if (Qa && "function" === typeof a.onModuleLoaded)
        a.onModuleLoaded(a); };
    ...
    if (a.ENVIRONMENT)
        ...
        ja = "object" === typeof window, fa = "function" === typeof importScripts, qa = "object" === typeof process && "function" === typeof require && !ja && !fa, za = !ja && !qa && !fa;
    if (qa) {
        var Aa, Ba;
        a.read = function (a, c) { Aa || (Aa = require("fs")); Ba || (Ba = require("path")); a = Ba.normalize(a); a = Aa.readFileSync(a); return c ? a : a.toString(); };
        ...
        process.on("uncaughtException", function (a) { if (!(a instanceof na))
            throw a; });
        process.on("unhandledRejection", function (a, c) { process.exit(1); });
        a.inspect = function () { return "[Emscripten Module object]"; };
    }
    else if (za)
        "undefined" != typeof read && (a.read = function (a) { return read(a); }), a.readBinary = function (a) { if ("function" === typeof readbuffer)
            return new Uint8Array(readbuffer(a)); a = read(a, "binary"); f("object" === typeof a); return a; }, "undefined" != typeof scriptArgs ? a.arguments = scriptArgs : "undefined" != typeof arguments &&
    ...
    pa = void 0;
    var oa = 0, Ia = "undefined" !== typeof TextDecoder ? new TextDecoder("utf8") : void 0;
    ...
    a.reallocBuffer || (a.reallocBuffer = function (a) { try {
        if (ArrayBuffer.transfer)
            var c = ArrayBuffer.transfer(D, a);
        else {
            var b = ia;
            c = new ArrayBuffer(a);
    ...
    try {
        ...
        Ta = function (a) { return a.byteLength; };
    }
    var Fa = a.TOTAL_STACK ||
        5242880, A = a.TOTAL_MEMORY || 16777216;
    A < Fa && a.printErr("TOTAL_MEMORY should be larger than TOTAL_STACK, was " + A + "! (TOTAL_STACK=" + Fa + ")");
    if (a.buffer)
        var D = a.buffer;
    else
        "object" === typeof WebAssembly && "function" === typeof WebAssembly.Memory ? (a.wasmMemory = new WebAssembly.Memory({ initial: A / 65536 }), D = a.wasmMemory.buffer) : D = new ArrayBuffer(A), a.buffer = D;
    r();
    E[0] = 1668509029;
    Ja[1] = 25459;
    if (115 !== W[2] || 99 !== W[3])
        throw "Runtime error: expected the system to be little-endian!";
    var Pa = [], Ma = [], Na = [], ob = [], Oa = [], La = !1, ea = 0, Ga = null, ra = null;
    a.preloadedImages = {};
    a.preloadedAudios = {};
    (function () {
        function e() { try {
        ...
        function b(b, e, d) {
            ...
            function S(b) { c().then(function (a) { return WebAssembly.instantiate(a, h); }).then(b).catch(function (c) { a.printErr("failed to asynchronously prepare wasm: " + c); O(c); }); }
            ...
            a.wasmBinary || "function" !== typeof WebAssembly.instantiateStreaming || Y(f) || "function" !== typeof fetch ? S(g) : WebAssembly.instantiateStreaming(fetch(f, { credentials: "same-origin" }), h).then(g).catch(function (c) { a.printErr("wasm streaming compile failed: " + c); a.printErr("falling back to ArrayBuffer instantiation"); S(g); });
            return {};
        }
        var d = "draco_decoder.wast", f = "draco_decoder.wasm", g = "draco_decoder.temp.asm.js";
        "function" === typeof a.locateFile && (Y(d) || (d = a.locateFile(d)), Y(f) || (f = a.locateFile(f)), Y(g) || (g = a.locateFile(g)));
        var h = { global: null, env: null, asm2wasm: { "f64-rem": function (a, c) { return a % c; }, "debugger": function () { debugger; } }, parent: a }, k = null;
        a.asmPreload = a.asm;
    ...
    var qb = ba;
    ba += 16;
    var y = { last: 0, caught: [], infos: {}, deAdjust: function (a) { if (!a || y.infos[a])
            return a; for (var c in y.infos)
            if (y.infos[c].adjusted === a)
                return c; return a; }, addRef: function (a) { a && y.infos[a].refcount++; },
        decRef: function (e) { if (e) {
            var c = y.infos[e];
            f(0 < c.refcount);
            c.refcount--;
            0 !== c.refcount || c.rethrown || (c.destructor && a.dynCall_vi(c.destructor, e), delete y.infos[e], ___cxa_free_exception(e));
        } }, clearRef: function (a) { a && (y.infos[a].refcount = 0); } }, w = { varargs: 0, get: function (a) { w.varargs += 4; return E[w.varargs - 4 >> 2]; }, getStr: function () { return u(w.get()); }, get64: function () { var a = w.get(), c = w.get(); 0 <= a ? f(0 === c) : f(-1 === c); return a; }, getZero: function () { f(0 === w.get()); } }, va = {}, Ha = 1;
    ka = function (a) {
        f(!Sa);
        var c = ba;
        ba = ba +
    ...
    a.asmGlobalArg = {};
    a.asmLibraryArg = { abort: O, assert: f, enlargeMemory: function () { var e = a.usingWasm ? 65536 : 16777216, c = 2147483648 - e; if (E[ka >> 2] > c)
            return !1; var b = A; for (A = Math.max(A, 16777216); A < E[ka >> 2];)
            A = 536870912 >= A ? ha(2 * A, e) : Math.min(ha((3 * A + 2147483648) / 4, e), c); e = a.reallocBuffer(A); if (!e || e.byteLength != A)
            return A = b, !1; a.buffer = D = e; r(); return !0; }, getTotalMemory: function () { return A; }, abortOnCannotGrowMemory: function () {
            ...
            if ("number" !== typeof b && "longjmp" !== b)
                throw b;
            a.setThrew(1, 0);
        } }, invoke_iii: function (e, c, b) { try {
            return a.dynCall_iii(e, c, b);
        }
        catch (p) {
            if ("number" !== typeof p && "longjmp" !== p)
                throw p;
            a.setThrew(1, 0);
        ...
        }, invoke_iiiiiii: function (e, c, b, d, f, g, h) { try {
        ...
        }, invoke_viii: function (e, c, b, d) { try {
            ...
            if ("number" !== typeof S && "longjmp" !== S)
                throw S;
            a.setThrew(1, 0);
        } }, invoke_viiii: function (e, c, b, d, f) { try {
            a.dynCall_viiii(e, c, b, d, f);
        }
        catch (xa) {
            if ("number" !== typeof xa && "longjmp" !== xa)
                throw xa;
            a.setThrew(1, 0);
        } }, invoke_viiiii: function (e, c, b, d, f, g) { try {
            a.dynCall_viiiii(e, c, b, d, f, g);
        }
        catch (ca) {
            if ("number" !== typeof ca && "longjmp" !== ca)
                throw ca;
            a.setThrew(1, 0);
        } }, invoke_viiiiii: function (e, c, b, d, f, g, h) { try {
        ...
        }, ___setErrNo: function (d) { a.___errno_location && (E[a.___errno_location() >> 2] = d); return d; }, ___syscall140: function (a, c) { w.varargs = c; try {
            var b = w.getStreamFromFD();
            w.get();
            var d = w.get(), e = w.get(), f = w.get();
            FS.llseek(b, d, f);
            E[e >> 2] = b.position;
        ...
        } }, ___syscall146: Z, ___syscall6: function (a, c) {
            w.varargs = c;
            try {
                var b = w.getStreamFromFD();
                FS.close(b);
        ...
        }, _abort: function () { a.abort(); }, _emscripten_memcpy_big: function (a, c, b) { W.set(W.subarray(c, c + b), a); return a; }, _llvm_trap: function () { O("trap!"); }, _pthread_getspecific: function (a) { return va[a] || 0; }, _pthread_key_create: function (a, c) { if (0 == a)
            return 22; E[a >> 2] = Ha; va[Ha] = 0; Ha++; return 0; }, _pthread_once: ma, _pthread_setspecific: function (a, c) { if (!(a in va))
            return 22; va[a] = c; return 0; }, flush_NO_FILESYSTEM: function () {
            var d = a._fflush;
            d && d(0);
            if (d = Z.printChar) {
                var c = Z.buffers;
        ...
        }, DYNAMICTOP_PTR: ka, tempDoublePtr: qb, ABORT: oa, STACKTOP: ta, STACK_MAX: ua };
    ...
    a.then = function (d) {
        if (a.calledRun)
            ...
            a.onRuntimeInitialized = function () { c && c(); d(a); };
        }
        return a;
    };
    na.prototype = Error();
    na.prototype.constructor = na;
    ra = function c() { a.calledRun || wa(); a.calledRun || (ra = c); };
    a.run = wa;
    a.exit = function (c, b) { if (!b || !a.noExitRuntime || 0 !== c) {
    ...
    a.NULL = T(0);
    a.destroy = function (a) { if (!a.__destroy__)
        throw "Error: Cannot destroy object. (Did you create it yourself?)"; a.__destroy__(); delete t(a.__class__)[a.ptr]; };
    a.compare = function (a, b) { return a.ptr === b.ptr; };
    a.getPointer = function (a) { return a.ptr; };
    a.getClass = function (a) { return a.__class__; };
    var l = { buffer: 0, size: 0, pos: 0, temps: [], needed: 0, prepare: function () { if (l.needed) {
            ...
            l.size += l.needed;
            l.needed = 0;
        } l.buffer || (l.size += 128, l.buffer = a._malloc(l.size), f(l.buffer)); l.pos = 0; }, alloc: function (c, b) { f(l.buffer); c = c.length * b.BYTES_PER_ELEMENT; c = c + 7 & -8; l.pos + c >= l.size ? (f(0 < c), l.needed += c, b = a._malloc(c), l.temps.push(b)) : (b = l.buffer + l.pos, l.pos += c); return b; }, copy: function (a, b, d) {
    ...
    F.prototype.GetValue = F.prototype.GetValue = function (a) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); return lc(c, a); };
    ...
    H.__cache__ = {};
    a.DracoUInt8Array = H;
    H.prototype.GetValue = H.prototype.GetValue = function (a) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); return rc(c, a); };
    ...
    I.prototype.GetValue = I.prototype.GetValue = function (a) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); return oc(c, a); };
    ...
    J.prototype.InitFromAttribute = J.prototype.InitFromAttribute = function (a) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); return !!rb(c, a); };
    ...
    x.prototype.InitFromAttribute = x.prototype.InitFromAttribute = function (a) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); return !!ub(c, a); };
    ...
    K.__cache__ = {};
    a.DracoInt8Array = K;
    K.prototype.GetValue = K.prototype.GetValue = function (a) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); return ic(c, a); };
    ...
    q.__cache__ = {};
    a.MetadataQuerier = q;
    q.prototype.HasEntry = q.prototype.HasEntry = function (a, b) { var c = this.ptr; l.prepare(); a && "object" === typeof a && (a = a.ptr); b = b && "object" === typeof b ? b.ptr : U(b); return !!Ec(c, a, b); };
    ...
    q.prototype.GetIntEntry = q.prototype.GetIntEntry = function (a, b) { var c = this.ptr; l.prepare(); a && "object" === typeof a && (a = a.ptr); b = b && "object" === typeof b ? b.ptr : U(b); return Bc(c, a, b); };
    q.prototype.HasDoubleEntry = q.prototype.HasDoubleEntry = function (a, b) { var c = this.ptr; l.prepare(); a && "object" === typeof a && (a = a.ptr); b = b && "object" === typeof b ? b.ptr : U(b); return !!Dc(c, a, b); };
    ...
    q.prototype.HasStringEntry = q.prototype.HasStringEntry = function (a, b) { var c = this.ptr; l.prepare(); a && "object" === typeof a && (a = a.ptr); b = b && "object" === typeof b ? b.ptr : U(b); return !!Gc(c, a, b); };
    q.prototype.GetStringEntry = q.prototype.GetStringEntry = function (a, b) { var c = this.ptr; l.prepare(); a && "object" === typeof a && (a = a.ptr); b = b && "object" === typeof b ? b.ptr : U(b); return u(Cc(c, a, b)); };
    q.prototype.NumEntries = q.prototype.NumEntries = function (a) {
        var c = this.ptr;
        a && "object" === typeof a && (a = a.ptr);
        return Hc(c, a);
    };
    q.prototype.GetEntryName = q.prototype.GetEntryName = function (a, b) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); b && "object" === typeof b && (b = b.ptr); return u(Ac(c, a, b)); };
    ...
    M.prototype.GetValue = M.prototype.GetValue = function (a) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); return $b(c, a); };
    ...
    g.__cache__ = {};
    a.Decoder = g;
    g.prototype.GetEncodedGeometryType = g.prototype.GetEncodedGeometryType = function (a) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); return Ub(c, a); };
    ...
    g.prototype.DecodeBufferToMesh = g.prototype.DecodeBufferToMesh = function (a, b) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); b && "object" === typeof b && (b = b.ptr); return T(Db(c, a, b), z); };
    g.prototype.GetAttributeId = g.prototype.GetAttributeId = function (a, b) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); b && "object" === typeof b && (b = b.ptr); return Kb(c, a, b); };
    ...
    g.prototype.GetAttributeIdByMetadataEntry = g.prototype.GetAttributeIdByMetadataEntry = function (a, b, d) { var c = this.ptr; l.prepare(); a && "object" === typeof a && (a = a.ptr); b = b && "object" === typeof b ? b.ptr : U(b); d = d && "object" === typeof d ? d.ptr : U(d); return Ib(c, a, b, d); };
    ...
    g.prototype.GetAttributeByUniqueId = g.prototype.GetAttributeByUniqueId = function (a, b) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); b && "object" === typeof b && (b = b.ptr); return T(Fb(c, a, b), n); };
    g.prototype.GetMetadata = g.prototype.GetMetadata = function (a) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); return T(Wb(c, a), R); };
    ...
    g.prototype.GetFaceFromMesh = g.prototype.GetFaceFromMesh = function (a, b, d) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); b && "object" === typeof b && (b = b.ptr); d && "object" === typeof d && (d = d.ptr); return !!Vb(c, a, b, d); };
    g.prototype.GetTriangleStripsFromMesh = g.prototype.GetTriangleStripsFromMesh = function (a, b) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); b && "object" === typeof b && (b = b.ptr); return Xb(c, a, b); };
    g.prototype.GetAttributeFloat = g.prototype.GetAttributeFloat = function (a, b, d) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); b && "object" === typeof b && (b = b.ptr); d && "object" === typeof d && (d = d.ptr); return !!Hb(c, a, b, d); };
    g.prototype.GetAttributeFloatForAllPoints = g.prototype.GetAttributeFloatForAllPoints = function (a, b, d) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); b && "object" === typeof b && (b = b.ptr); d && "object" === typeof d && (d = d.ptr); return !!Gb(c, a, b, d); };
    ...
    g.prototype.GetAttributeInt8ForAllPoints = g.prototype.GetAttributeInt8ForAllPoints = function (a, b, d) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); b && "object" === typeof b && (b = b.ptr); d && "object" === typeof d && (d = d.ptr); return !!Nb(c, a, b, d); };
    ...
    g.prototype.GetAttributeInt16ForAllPoints = g.prototype.GetAttributeInt16ForAllPoints = function (a, b, d) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); b && "object" === typeof b && (b = b.ptr); d && "object" === typeof d && (d = d.ptr); return !!Lb(c, a, b, d); };
    ...
    g.prototype.GetAttributeInt32ForAllPoints = g.prototype.GetAttributeInt32ForAllPoints = function (a, b, d) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); b && "object" === typeof b && (b = b.ptr); d && "object" === typeof d && (d = d.ptr); return !!Mb(c, a, b, d); };
    ...
    g.prototype.SkipAttributeTransform = g.prototype.SkipAttributeTransform = function (a) { var c = this.ptr; a && "object" === typeof a && (a = a.ptr); Yb(c, a); };
    ...
    N.prototype.GetValue = N.prototype.GetValue =
        function (a) { var b = this.ptr; a && "object" === typeof a && (a = a.ptr); return fc(b, a); };
