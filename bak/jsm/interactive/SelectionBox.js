...
var SelectionBox = (function () {
	...
	SelectionBox.prototype.searchChildInFrustum = function (frustum, object) {
		// if (this.collection.some(obj => obj.uuid === object.uuid)) return;
		if (object.isMesh || object.isLine || object.isPoints) {
			...
			if (object.material !== undefined) {
				...
				if (object.isInstancedMesh) {
					...
					let _instanceWorldMatrix = new Matrix4();
					var matrixWorld = object.matrixWorld;
					var raycastTimes = object.count;

					if (object.material === undefined) return;

					for (var instanceId = 0; instanceId < raycastTimes; instanceId++) {
						// calculate the world matrix for each instance
						object.getMatrixAt(instanceId, _instanceLocalMatrix);
						_instanceWorldMatrix.multiplyMatrices(matrixWorld, _instanceLocalMatrix);

						if (object.geometry.boundingSphere === null) object.geometry.computeBoundingSphere();
						center.copy(object.geometry.boundingSphere.center);
	
						center.applyMatrix4(_instanceWorldMatrix);
		
						if (frustum.containsPoint(center)) {
							if (this.collection.some(obj => obj.uuid === object.uuid)){
								this.collection.find(obj => obj.uuid === object.uuid).userData.instanceIds.push(instanceId);
							}else{
								object.userData.instanceIds = [instanceId];
